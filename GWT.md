...
`gwt` launches a fuzzy-filterable TUI listing existing worktrees and remote branches. Selecting a worktree outputs its path to stdout. Selecting a remote branch confirms, creates a worktree tracking it, then outputs the new path. A shell wrapper function handles the actual `cd`.

Worktree location defaults to `.claude/worktrees/` relative to the git root. Configurable via `gwt_WORKTREE_DIR` env var (will move to `gwt.toml` in a future version).

## Commands

### `gwt` (default, no subcommand)
...
### `gwt add <name> [--track <remote-branch>]`

Non-interactive worktree creation.

- `gwt add my-feature` → `git worktree add .claude/worktrees/my-feature`
- `gwt add review/fix-bug --track origin/fix-bug` → `git worktree add .claude/worktrees/review/fix-bug --track origin/fix-bug`
...
The shell wrapper sets `gwt_WRAPPED=1` so the binary can detect it. If the binary runs without `gwt_WRAPPED`, print a hint to stderr on first run suggesting `gwt init`.
...
- `gwt.toml` config file (worktree path, copy/link rules)
